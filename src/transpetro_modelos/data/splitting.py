...
def temporal_split(
    ...
    - If val_start_date is provided:
        ...
        - val:   data from val_start_date to cutoff (or to val_end_date, if provided;
          rows between val_end_date and cutoff are then excluded from all splits)
    - Otherwise:
    ...
    if val_start_date is not None:
        ...
        if val_end_date is not None:
            val_data = val_data[val_data.index < pd.Timestamp(val_end_date)]
    else:
        n = len(normal_data)
